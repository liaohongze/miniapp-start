      ...
      success(r1) {
        if (r1.code) {
          ...
          req.code2sessionId(r1.code)
            .then((r2) => {
              const newSessionId = r2.access_token;
              ...
              //   data: newSessionId,
              // });
              wx.setStorageSync(sessionStorageKey, newSessionId);
              wx.setStorageSync("userInfo", r2);
              res(r2.access_token);
            })
            .catch(rej);
        } else {
        ...
        .then(() => {
          ...
          requestP(options)
            .then((r2) => {
              ...
              if (!req.isSessionAvailable(r2, sessionStorageKey)) {
                /**
                 * 登录状态无效，则重新走一遍登录流程
                 * 销毁本地已失效的sessionId
                 */
